...
Modified by Lim:
20241029
    - Customized processing for mapping by Lim to reescue undetected ones
    - outdir corection to be able to export to the current folder
    - create missing featurefiles in map_element_barcodes
*/
...
// proper handling of --min-frac option
if (params.containsKey("min-frac")){
    params.min_frac=params['min-frac']
}else{
    params.min_frac="0.5"
}
...
process 'map_element_barcodes' {
    ...
    shell:
        ...
        python ${"$baseDir"}/src/nf_ori_map_barcodes_limlab.py ${"$baseDir"} ${fastq_bc} ${count_fastq} \
        $bam ${count_bam} ${name} 0 30 n
